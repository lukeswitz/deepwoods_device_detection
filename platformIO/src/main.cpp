...
// ------------ Counts & Baseline Storage ------------
static volatile size_t currBLECount   = 0,
                      currWiFiCount  = 0,
                      currProbeCount = 0;
static std::vector<String> baselineBLE, baselineWiFi, baselineProbe;
static std::vector<String> curProbeSeen;
...
static void IRAM_ATTR snifferCallback(void* buf, wifi_promiscuous_pkt_type_t type) {
  ...
  // parse SSID IE
  ev.ssid[0] = '\0';
  const uint8_t *ptr = hdr + 24;
  const uint8_t *end = p->payload + p->rx_ctrl.sig_len;
  while (ptr + 1 < end) {
    uint8_t id = ptr[0], len = ptr[1];
    if (ptr + 2 + len > end) break;
    if (id == 0) { // SSID IE
      uint8_t L = min((uint8_t)32, len);
      memcpy(ev.ssid, ptr + 2, L);
      ev.ssid[L] = '\0';
      break;
    }
    ptr += 2 + len;
  }
  if (ev.ssid[0] == '\0') strcpy(ev.ssid, "<wildcard>");
...
void ProbeTask(void*) {
  ...
  while (xQueueReceive(probeQ, &ev, portMAX_DELAY)) {
    String entry = String(ev.mac) + "|" + String(ev.ssid);
    if (std::find(curProbeSeen.begin(), curProbeSeen.end(), entry) == curProbeSeen.end()) {
      curProbeSeen.push_back(entry);
      currProbeCount = curProbeSeen.size();
      if (!isBaseline &&
          std::find(baselineProbe.begin(), baselineProbe.end(), entry) == baselineProbe.end()) {
...
void WiFiTask(void*) {
  ...
  while (true) {
    ...
    if (n > 0) {
      for (int i = 0; i < n; i++) {
        ...
        if (std::find(seen.begin(), seen.end(), b) == seen.end()) {
          ...
          if (!isBaseline &&
              std::find(baselineWiFi.begin(), baselineWiFi.end(), b) == baselineWiFi.end()) {
    ...
    // Re‑enable promiscuous sniffing for ProbeReqs
...
void setup() {
  ...
  // Wi‑Fi promiscuous sniff in STA mode
